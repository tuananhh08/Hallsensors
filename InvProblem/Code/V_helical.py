...
SENSITIVITY = 7.5e-3 / 1e-3      # 0.75 mV/mT -> V/T
m0          = 1.0
...
helical_file = Path(r"D:\Downloads\Hallsensors\InvProblem\PosData\Helical_points_coordinates.csv")

# output path
out_file = BASE_DIR / "Helical_voltage.csv"
...
# LOAD HELICAL DATA
df = pd.read_csv(helical_file)

helical_xyz = df.iloc[:, :3].to_numpy()
cos_alpha  = df["cos_alpha"].to_numpy()
cos_beta    = df["cos_beta"].to_numpy()

print(f"Loaded {len(helical_xyz)} helical points")

# COMPUTE MAGNETIC MOMENT
m_vecs = compute_m_vectors(cos_alpha, cos_beta)

# COMPUTE B
B_all = compute_B_all(helical_xyz, sensor_pos, m_vecs)
